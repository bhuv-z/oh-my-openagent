/**
 * Gemma-specific overlay sections for Sisyphus prompt.
 *
 * Design principles:
 ...
 * - Verification enforcement to counter optimistic completion tendency
 * - Concrete examples to guide tool call patterns
 * - Gemma uses reasoningEffort instead of thinking.budgetTokens
 */

export function buildGemmaToolCallEnforcement(): string {
...
- Prefer \`edit\` and \`write\` tools over \`apply_patch\` (may be unreliable on some deployments)
...
export function buildGemmaToolGuide(): string {
  return `<GEMMA_TOOL_GUIDE>
...
export function buildGemmaToolCallExamples(): string {
  return `<GEMMA_TOOL_CALL_EXAMPLES>
...
\`\`\`
</GEMMA_TOOL_CALL_EXAMPLES>`;
}

export function buildGemmaDelegationReinforcement(): string {
...
export function buildGemmaVerificationOverride(): string {
  return `<GEMMA_VERIFICATION_OVERRIDE>
...
</GEMMA_VERIFICATION_OVERRIDE>`;
}

export function buildGemmaDependencyAndAskGate(): string {
...
export function buildGemmaExecutionLoop(): string {
  return `<GEMMA_EXECUTION_LOOP>
...
5. **VERIFY** — See \`<GEMMA_VERIFICATION_LOOP>\` below. Not optional.
...
7. **DONE** — See \`<GEMMA_COMPLETENESS_CONTRACT>\` below. Exit only when ALL conditions met.
</GEMMA_EXECUTION_LOOP>

<GEMMA_VERIFICATION_LOOP>
...
export function buildGemmaOutputContract(): string {
  return `<GEMMA_OUTPUT_CONTRACT>
## Output Format
...
</GEMMA_OUTPUT_CONTRACT>`;
}

export function buildGemmaIntentGateEnforcement(): string {
  return `<GEMMA_INTENT_GATE_ENFORCEMENT>
